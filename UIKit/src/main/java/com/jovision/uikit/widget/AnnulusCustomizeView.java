...
public class AnnulusCustomizeView extends View {
    ...
    public AnnulusCustomizeView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (aIndex == R.styleable.AnnulusCustomizeView_annulusWidth) {
                mAnnulusWidth = a.getDimensionPixelSize(aIndex,
                        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                                10,
                                getResources().getDisplayMetrics()));
            } else if (aIndex == R.styleable.AnnulusCustomizeView_annulusColor) {
                mAnnulusColor = a.getColor(aIndex, Color.BLACK);
            } else if (aIndex == R.styleable.AnnulusCustomizeView_loadColor) {
                mLoadColor = a.getColor(aIndex, Color.BLACK);
            } else if (aIndex == R.styleable.AnnulusCustomizeView_textColor) {
                mTextColor = a.getColor(aIndex, Color.BLACK);
            } else if (aIndex == R.styleable.AnnulusCustomizeView_textSize) {
                mTextSize = a.getDimensionPixelSize(aIndex,
                        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                                15,
                                getResources().getDisplayMetrics()));
            } else if (aIndex == R.styleable.AnnulusCustomizeView_progressType) {
                mProgressType = a.getInt(aIndex, 1);
            } else if (aIndex == R.styleable.AnnulusCustomizeView_isShowText) {
                mIsShowText = a.getInt(aIndex, 1);
            } else if (aIndex == R.styleable.AnnulusCustomizeView_progressA) {
